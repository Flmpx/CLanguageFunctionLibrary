...
//释放Data数据
void freeMDataInMOAMap(Data_M* data) {
    //为空不释放
...
void freeMEntryInMOAMap(Entry_M_inOAMap* entry) {
    ...
    freeMDataInMOAMap(&(entry->key));
    freeMDataInMOAMap(&(entry->value));
    entry->isEmpty = true;
}
...
static Entry_M_inOAMap copyMEntry(Entry_M_inOAMap oldEntry) {
    ...
    if (newEntry.value.isEmpty) {
        freeMDataInMOAMap(&(newEntry.key));
        return getEmptyMEntry();
    }
...
static InfoOfReturn addMEntryFunction(OAMap_M* pMap, Data_M key, Data_M value) {
    ...
    while (pMap->arr[index].state != NONE_IN_MAP && pMap->arr[index].state != DEL_IN_MAP) {
        ...
        if (compareMKey(&(pMap->arr[index].key), &key) == 0) {
            ...
            freeMDataInMOAMap(&(pMap->arr[index].value));
...
void printMDataInMOAMap(Data_M data, char* tip) {
    if (data.isEmpty) {
