...
extern void initSList(DList_S* plist, InfoOfData* valInfo);


/// @brief 释放SData(Data_S类型)的内容
/// @param plist DList_S的指针
/// @param inputData Data_S类型指针
extern void freeSDataInSList(DList_S* plist, Data_S* inputData);
...
extern Data_S getPtrSDataBySDataInSList(DList_S* plist, void* data, void* content);

/// @brief 通过Pos位置返回Data_S的,注意这里Data_S中的void* data和void* content都是复制的,使用完后记得释放
/// @param plist DList_S类型指针
/// @param pos 位置(从0开始)
/// @return 返回Data_S类型数据, 这里是SData, 如果没有, 返回空Data_S类型数据, 通过Data.isEmpty进行查看, 具体查看文档
extern Data_S getCopySDataByPosInSList(DList_S* plist, int pos);
...
/// @param pos 位置(从0开始)
/// @return 返回Data_S类型数据, 这里是SData, 如果没有, 返回空Data_S类型数据, 通过Data.isEmpty进行查看, 具体查看文档
extern Data_S getPtrSDataByPosInSList(DList_S* plist, int pos);
...
extern bool hasSDataInSList(DList_S* plist, void* data, void* content);
...
extern InfoOfReturn insertSDataAtEndInSList(DList_S* plist, void* data, void* content);
...
/// @param content void* content
/// @return 返回InfoOfRetrun中的枚举类型, 具体查看文档
extern InfoOfReturn insertSDataAtStartInSList(DList_S* plist, void* data, void* content);
...
/// @return 返回InfoOfRetrun中的枚举类型, 具体查看文档
extern InfoOfReturn insertSDataAtPosInSList(DList_S* plist, void* data, void* content, int pos);

/// @brief 删除DList_S链表尾节点
/// @param plist DList_S链表指针
/// @return 返回InfoOfRetrun中的枚举类型, 具体查看文档
extern InfoOfReturn delEndNodeInSList(DList_S* plist);


/// @brief 删除DList_S链表头节点
/// @param plist DList_S链表指针
/// @return 返回InfoOfRetrun中的枚举类型, 具体查看文档
extern InfoOfReturn delStartNodeInSList(DList_S* plist);
...
/// @param content void* content
/// @return 返回InfoOfRetrun中的枚举类型, 具体查看文档
extern InfoOfReturn delNodeBySDataInSList(DList_S* plist, void* data, void* content);

/// @brief 通过位置删除DList_S的节点
/// @param plist DList_S链表指针
/// @param pos 要删除的位置(从0开始)
/// @return 返回InfoOfRetrun中的枚举类型, 具体查看文档
extern InfoOfReturn delNodeByPosInSList(DList_S* plist, int pos);

/// @brief 反转DList_S链表
/// @param plist DList_S链表指针
extern void reverseSList(DList_S* plist);
...
extern void printSDataInSList(DList_S* plist, Data_S inputData);


/// @brief 打印DList_S链表数据
/// @param plist DList_S链表指针
extern void printSList(DList_S* plist);

/// @brief 清除DList_S链表
/// @param plist DList_S链表指针
extern void freeSList(DList_S* plist);
#endif
