...
extern void freeMDataInMDList(Data_M* inputData);
...
extern Data_M getPtrMDataByMDataInMDList(DList_M* plist, Data_M inputData);

/// @brief 通过Pos位置返回MData(Data_M类型)的,注意这里MData中的void* data和void* content都是复制的,使用完后记得释放
/// @param plist DList_M类型指针
/// @param pos 位置(从0开始)
/// @return 返回Data_M类型数据, 这里是MData, 如果没有, 返回空Data_M类型数据, 通过Data.isEmpty进行查看, 具体查看文档
extern Data_M getCopyMDataByPosInMDList(DList_M* plist, int pos);
...
/// @param pos 位置(从0开始)
/// @return 返回Data_M类型数据, 这里是MData, 如果没有, 返回空Data_M类型数据, 通过Data.isEmpty进行查看, 具体查看文档
extern Data_M getPtrMDataByPosInMDList(DList_M* plist, int pos);

/// @brief 判断Data_M数据是否在DList_M里面
/// @param plist DList_M类型指针
/// @param inputData Data_M类型数据
/// @return 有就返回true,没有就false
extern bool hasMDataInMDList(DList_M* plist, Data_M inputData);
...
extern InfoOfReturn insertMDataAtEndInMDList(DList_M* plist, Data_M inputData);
...
/// @param inputData Data_M类型数据
/// @return 返回InfoOfRetrun中的枚举类型, 具体查看文档
extern InfoOfReturn insertMDataAtStartInMDList(DList_M* plist, Data_M inputData);
...
/// @return 返回InfoOfRetrun中的枚举类型, 具体查看文档
extern InfoOfReturn insertMDataAtPosInMDList(DList_M* plist, Data_M inputData, int pos);
...
/// @param inputData Data_M类型数据
/// @return 返回InfoOfRetrun中的枚举类型, 具体查看文档
extern InfoOfReturn delNodeByMDataInMDList(DList_M* plist, Data_M inputData);
...
extern Data_M stackMDataInMDList(void* data, void* content, int type, InfoOfData* dataInfo);
...
/// @param inputData Data_M类型数据
extern void printSDataInMDList(Data_M inputData);
